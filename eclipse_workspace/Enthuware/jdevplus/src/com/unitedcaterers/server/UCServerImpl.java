 ...
 * The networked (RMI/Socket based) and non-networked servers, both, extend from this class.
 * The benefit of this approach is that the client can instantiate either a networked server
 * or a non-networked server without having any special code for either of them. Further, we can build
 * higher level operations using low level calls supported by the DataBase class in this class, which
 * will become available to both kinds of servers. For example, you can implement a method say,
 * public boolean bookCaterer(int recordNo, String customerid)
 * {
 *  //lock
 *  //read
 *  //modify
 ...
 * }
 *
 * The above code will be same for both the servers whether it is RMI or Socket. Implementing
 * it in the abstract class will let other adapters inherit it.
 *
...
public class UCServerImpl implements UCServer {
    ...
    public boolean bookCaterer(String customerid, String[] originalData) throws RemoteException, UCException {
        ...
        try {
            ...
            if(data[5] == null || data[5].trim().length() == 0) {
                ...
                status = true;
            } else throw new UCException("This caterer is already booked.");
            ...
            try {
                if(lockkey != null) db.unlock(recordNo, lockkey);
            } catch(Exception e) {
    ...
    public String[][] findAndReturnData(String[] criteria) throws UCException {
        ...
        try {
            ...
            //goes ahead and updates(or deletes) record 5. So when User A, tries
            ...
            //Here we are using the pessimistic approach because it is easier to understand,
            //implement, and debug. However, its performance is less than optimal.
            ...
            try {
                if(lockkey != null) db.unlock(-1, lockkey);
            } catch(Exception e) {
                logger.log(Level.SEVERE, "Serious problem in unlocking. Should not happen.", e);
            }
        }
     ...
     * Ideally, the DataBase interface should have a close() method. Since it does not, db needs to be cast to DataBaseImpl
     */
    public void close() {
