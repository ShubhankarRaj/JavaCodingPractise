			...
			while(!allDigitsCovered){
				...
				if (product == 0){
					...
					for(int i = 0; i<digitsInProd.length; i++){
						String noPresentInMap = trackDigitVisited.get(Integer.parseInt(Character.toString(digitsInProd[i])));
						if(noPresentInMap.equals("NOTFOUND")){
							trackDigitVisited.put(Integer.parseInt(Character.toString(digitsInProd[i])), "DIGIT FOUND");
						}
					}
