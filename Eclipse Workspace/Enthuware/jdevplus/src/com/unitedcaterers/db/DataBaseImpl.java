...
public class DataBaseImpl implements DataBase {
    ...
    /**
     * This is the only constructor. It validates the magiccode at the begining of the
     * database file and then if valid, initializes the internal structures.
     * @param dbfilename The full file name (including path) that stores the data.
     * @param magiccode The magiccode that identifies the file as the file meant for a given
     * application.
     * @throws IOException This exception is thrown if there is a problem in reading/writing
     * the db file or if the given magiccode does not match with the magiccode stored
     * in the dbfile.
     */
    public DataBaseImpl(String dbfilename, String magiccode) throws IOException {
        ...
        logger.fine("Magiccode in file : " + str + " Magiccode given : " + magiccode + (valid ? "DB File is valid" : "Mismatch in magiccode"));
        if (!valid) {
        ...
        for (int i = 0; i < nooffields; i++) {
            ...
            tempRecordlength = tempRecordlength + fl;
            logger.fine("Field Name : " + str + " Field Length : " + fl);
            fieldnames[i] = str;
            fieldmap.put(str, new Short(fl));
        }
        tempRecordlength = tempRecordlength + 2;//2 bytes for deleted flag.
        
        recordlength = tempRecordlength;
        dis.close();
        fis.close();
     ...
     * @param recNo The record to be returned. From 0 to N
    ...
    private String[] parseRecord(String recorddata) {
        ...
        for (int i = 0; i < fieldnames.length; i++) {
            int fieldlength = ((Short) fieldmap.get(fieldnames[i])).intValue();
    ...
    /**
     * This method takes the record number, record data, and the lockkey and update the record.
     * @param recNo The record number to be updated
     * @param data New data for the record
     * @param lockkey The record should have been locked with this key.
     */
    @Override
    public synchronized void update(int recNo, String[] data, String lockkey) throws RecordNotFoundException, SecurityException {
        if (recNo < 0) {
            throw new RecordNotFoundException("No such record : " + recNo);
        }
    ...
    private byte[] getByteArray(String[] data) throws IOException {
        ...
        for (int i = 0; i < fieldnames.length; i++) {
            ...
            short flength = ((Short) fieldmap.get(fieldnames[i])).shortValue();
            ca = new char[flength];
     ...
     * @param recNo The record number that is to be deleted.
     * @param lockkey The record should have been locked using this key.
     * @throws RecordNotFoundException Thrown if this record number does not exist.
     * @throws java.lang.SecurityException Thrown if the lockkey does not match the key with
     * which this record was locked.
     */
    @Override
    public synchronized void delete(int recNo, String lockkey) throws RecordNotFoundException, SecurityException {
        if (recNo < 0) {
            throw new RecordNotFoundException("No such record : " + recNo);
        }
        if (lockkey == null) {
        ...
        if (lockkey.equals(realkey)) {
            try {
                ...
                throw new SecurityException("Unable to delete the record : " + recNo +
                        " : " + e.getMessage());
            }
        }
    }
    ...
    @Override
    public synchronized void deleteWithoutCookie(int recNo) throws RecordNotFoundException, SecurityException {
        if (recNo < 0) {
            throw new RecordNotFoundException("No such record : " + recNo);
        }
        String realkey = (String) locker.getOwner(recNo);
    ...
    public synchronized int[] find(String[] criteria) {
        ...
        try {
            ...
            for (int i = 0; i < noofmatches; i++) {
                retvalue[i] = ((Integer) matchingindices.get(i)).intValue();
            }
            return retvalue;
        } catch (Exception e) {
            e.printStackTrace();
    ...
    /**
     * This method creates a new record with the given data.
     * It uses slots occupied by previously deleted records if available.
     * @param data The String array containing all the fields.
     * @return record number.
     * @throws DuplicateKeyException Thrown if a record with same values for all the fields already exists.
     */
    @Override
    ...
    /**
     * Locks the record. If the record has been locked, it waits until it gets the lock.
     *
     * A client should not try to lock a record that it has already locked since it will lead to a deadlock.
     * This method does not attempt to check or prevent deadlock arising because of multiple
     * client requesting locks in cyclic pattern. For example, let's say Client A holds lock for Rec3 and
     * Client B hold lock for Rec 5. Now, if Client A requests lock for Rec 5 and Client B requests
     * lock for Rec 3, there will be a deadlock.
     * In general, locking is done by the application and so deadlock problem should be taken care of
     * by the application and not by the database. The Database has no knowledge of who is requesting the
     * lock for what reason. Application code knows that and is thus better prepared to prevent deadlocks.
     * @param recNo
     * @return The lock key.
     * @throws RecordNotFoundException
     */
     ...
     * @param lockkey The record (or the database) should have been locked using this key.
     * @throws RecordNotFoundException Thrown if there is no record by this record number.
     * @throws java.lang.SecurityException Thrown if the lockkey does not match the key with which the record was locked.
     */
    @Override
    ...
    /**
     * This method is similar to lock() method except that it does not return lock cookie.
     * This method assumes that the client that holds the lock is the one that tries to unlock it.
     * The remote layer (RMIUCServer or SocketUCServer) implements the mechanism that ensures that
     * only the client that holds the lock is able to unlock it.
     * @param recNo
     * @throws RecordNotFoundException
     */
    @Override
    ...
    /**
     * This method is similar to unlock() method except that it does not take lock cookie parameter.
     * This method assumes that the client that holds the lock is the one that tries to unlock it.
     * The remote layer (RMIUCServer or SocketUCServer) implements the mechanism that ensures that
     * only the client that holds the lock is able to unlock it.
     * @param recNo Record to be unlocked. -1 implies unlock the database.
     ...
     */
    @Override
    public void unlockWithoutCookie(int recNo) throws RecordNotFoundException {
        locker.unlockWithoutCookie(recNo);
    }

    @Override
    public synchronized void updateWithoutCookie(int recNo, String[] data) throws RecordNotFoundException, SecurityException {
        if (recNo < 0) {
            throw new RecordNotFoundException("No such record : " + recNo);
        }
        if (data == null || data.length != fieldnames.length) {
    ...
    /**
     * This Inner class implements a primitive lock manager. It keeps a hashtable that stores the record no. as key and
     * an Object (place holder) as value that denotes that somebody is using the record.
     */
    private class LockManager {
        ...
        public String getOwner(int recordNo) {
            return (String) locks.get("" + recordNo);
        }

        /**
         * This method waits till all the locks (entries in the locks HashMap) are released.
         * It then sets the dblocked flag and returns.
         */
        private synchronized String lockDB() {
        ...
        /**
         * This method waits till the locks HashMap returns null for the requested record no.
         ...
         */
        public synchronized String lock(int recordNo) {
            if (recordNo == -1) {
                return lockDB();
            }

            String key = (String) locks.get("" + recordNo);
        ...
        /**
         * This method removes the value stored in the locks HashMap for recordNo.
         * and notifies all the threads waiting on 'this'.
         *
         * @param int recordNo The record no. to be unlocked.
         */
        public synchronized void unlock(int recordNo, String lockkey) throws SecurityException {
            if (recordNo == -1) {
                if (lockkey != null && lockkey.equals(dbkey)) {
                    dblocked = false;
            ...
            if (key != null && lockkey.equals(key)) {
         ...
         * and notifies all the threads waiting on 'this'.
         *
         * Observe that this method has no way of knowing whether the client who has
         * locked the record is the one trying to unlock it. This functionality is to be
         * implemented by a higher level service. (See package serverwithoutlockcookie.)
         *
         * @param int recordNo The record no. to be unlocked.
         */
        public synchronized void unlockWithoutCookie(int recordNo) {
            if (recordNo == -1) {
                dblocked = false;
                notifyAll();
                return;
     ...
     * It implements a thread that continuously acquires and releases locks on
     * random record numbers including -1 (i.e. db lock)
     * Create multiple instances of this class and start them to simulate multiple
     * clients trying to acquire locks.
     */
    public static class LockTestThread extends Thread {

        private String name = "";
        private DataBaseImpl dbi;
        ...
        public void run() {
            try {
                while (true) {
                    int recno = (int) (Math.random() * 10) - 1; //substract 1 so that recno can also be -1 for database lock.
